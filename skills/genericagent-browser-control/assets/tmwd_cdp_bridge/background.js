...
importScripts('config.js', 'cookie_utils.js');
...
async function getAllowExternalControl() {
...
function getControlStateData(allowExternalControl) {
  return {
...
async function setAllowExternalControl(value) {
  const allowExternalControl = value !== false;
  await chrome.storage.local.set({ [CONTROL_STORAGE_KEY]: allowExternalControl });
  if (allowExternalControl) {
    connectWS();
  } else {
    disconnectWS();
  }
  return allowExternalControl;
}

async function ensureExternalControlAllowed(sender) {
  if (isExtensionPageSender(sender)) return true;
  return await getAllowExternalControl();
}

async function handleExtMessage(msg, sender) {
  if (msg.cmd === 'controlState') {
    if (msg.method === 'get') {
      return { ok: true, data: getControlStateData(await getAllowExternalControl()) };
    }
    if (msg.method === 'set') {
      const allowExternalControl = await setAllowExternalControl(msg.allow);
      return { ok: true, data: getControlStateData(allowExternalControl) };
    }
    return { ok: false, error: 'Unknown method: ' + msg.method };
  }

  if (!(await ensureExternalControlAllowed(sender))) {
    return { ok: false, error: EXTERNAL_CONTROL_DISABLED_ERROR };
  }
...
async function handleCookies(msg, sender) {
  try {
    ...
    const origin = TMWDCookieUtils.getTopLevelSite(url);
    if (!origin) {
      return { ok: false, error: TMWDCookieUtils.cookieErrorMessage(url) };
    }
...
async function handleWsExec(data) {
  if (!(await getAllowExternalControl())) {
    ws.send(JSON.stringify({ type: 'error', id: data.id, error: EXTERNAL_CONTROL_DISABLED_ERROR }));
    return;
  }
...
function connectWS() {
  ...
  ws = null;
  getAllowExternalControl().then((allowExternalControl) => {
    ...
    console.error('[TMWD-WS] Failed to read control state:', error);
    scheduleProbe();
  });
}
...
async function sendTabsUpdate() {
  ...
  if (!(await getAllowExternalControl())) return;
